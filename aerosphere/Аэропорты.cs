...
{
    public partial class Аэропорты : Form
    {
        ...
        private void LoadAiroportsToDgv()
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = $"SELECT * FROM [Аэропорты]";
        ...
        private void Аэропорты_Load(object sender, EventArgs e)
        {
            LoadAiroportsToDgv();
        }
        ...
        {
            string AirpotName = textBox1.Text.Trim();

            try
            {
                if (string.IsNullOrEmpty(AirpotName))
                {
                ...
                else
                {
                    ...
                    dataTable.DefaultView.RowFilter = $"Имя LIKE '%{AirpotName}%'";
                }
